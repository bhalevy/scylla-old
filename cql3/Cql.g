...
alterTableStatement returns [shared_ptr<alter_table_statement> expr]
    ...
    : K_ALTER K_COLUMNFAMILY cf=columnFamilyName
          ...
          | K_ADD                                     { type = alter_table_statement::type::add; }
            (          id=cident   v=comparatorType   s=cfisStatic { column_changes.emplace_back(id,  v,  s);  }
            | '('     id1=cident  v1=comparatorType  s1=cfisStatic { column_changes.emplace_back(id1, v1, s1); }
                 (',' idn=cident  vn=comparatorType  sn=cfisStatic { column_changes.emplace_back(idn, vn, sn); } )* ')'
            )
          | K_DROP                                    { type = alter_table_statement::type::drop; }
            (          id=cident { column_changes.emplace_back(id);  }
            | '('     id1=cident { column_changes.emplace_back(id1); }
                 (',' idn=cident { column_changes.emplace_back(idn); } )* ')'
            )
