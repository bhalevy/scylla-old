...
void alter_table_statement::add_column(const schema_ptr schema, const table& cf, schema_builder& cfm, std::vector<view_ptr>& view_updates, const shared_ptr<column_identifier> column_name, const shared_ptr<cql3_type> validator, const column_definition* def, bool is_static) {
    if (is_static) {
...
void alter_table_statement::alter_column(const schema_ptr schema, const table& cf, schema_builder& cfm, std::vector<view_ptr>& view_updates, const shared_ptr<column_identifier> column_name, const shared_ptr<cql3_type> validator, const column_definition* def, bool is_static) {
    if (!def) {
...
void alter_table_statement::drop_column(const schema_ptr schema, const table& cf, schema_builder& cfm, std::vector<view_ptr>& view_updates, const shared_ptr<column_identifier> column_name, const shared_ptr<cql3_type> validator, const column_definition* def, bool is_static) {
    if (!def) {
        throw exceptions::invalid_request_exception(sprint("Column %s was not found in table %s", column_name, column_family()));
    }
