...
private:
    static interval_type make_interval(const schema& s, const dht::partition_range& range) {
        return interval_type::closed(
                compatible_ring_position(s, dht::ring_position_view(range.start()->value())),
                compatible_ring_position(s, dht::ring_position_view(range.end()->value())));
    }
    ...
    static interval_type make_interval(const schema& s, const sstable& sst) {
        return interval_type::closed(
                compatible_ring_position(s, dht::ring_position(sst.get_first_decorated_key())),
                compatible_ring_position(s, dht::ring_position(sst.get_last_decorated_key())));
    }
...
public:
    static dht::ring_position to_ring_position(const compatible_ring_position& crp) {
...
private:
    ...
    static bool is_before_interval(const compatible_ring_position& crp, const interval_type& interval) {
    ...
    void maybe_invalidate_iterator(const compatible_ring_position& crp) {
...
public:
    ...
    virtual std::tuple<dht::partition_range, std::vector<shared_sstable>, dht::ring_position_view> select(const dht::ring_position_view& pos) override {
        auto crp = compatible_ring_position(*_schema, pos);
