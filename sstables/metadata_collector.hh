...
struct column_stats {
    ...
    void update_ldt_and_ts_histogram(int32_t value) {
        local_deletion_time_tracker.update(value);
    ...
    void update(const deletion_time& dt) {
        ...
        update_ldt_and_ts_histogram(dt.local_deletion_time);
    }
    void do_update(const tombstone& t) {
        update_timestamp(t.timestamp);
        update_ldt_and_ts_histogram(t.deletion_time.time_since_epoch().count());
    }
