...
future<sstring> sstable::set_generation_setup(unsigned long new_generation) const {
    if (_sst_dir && _sst_dir->size() != _dir.size()) {
        auto dst = sst_dir(_dir, new_generation);
...
future<> sstable::set_generation_cleanup(unsigned long new_generation) {
    ...
    if (_sst_dir && _sst_dir->size() != _dir.size()) {
        _sst_dir = sst_dir(_dir, new_generation);
    }
    return make_ready_future<>();
}
...
sstable::final_remove_by_toc(sstring dir, sstring tmp_toc_name, unsigned long gen, const io_error_handler& error_handler) {
    return sstable_io_check(error_handler, remove_file, tmp_toc_name).then([dir, tmp_toc_name, gen, &error_handler] {
        return fsync_directory(error_handler, dir).then([dir, tmp_toc_name, gen, &error_handler] {
            if (basename(dir) == sst_dir_basename(gen)) {
                return remove_file(dir).then([error_handler, parent = dirname(dir)] {
                    return fsync_directory(error_handler, parent);
