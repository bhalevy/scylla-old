...
sstable::~sstable() {
    ...
    if (_marked_for_deletion) {
        ...
        try {
            ...
            // POR to fix that is by passing large_data_handler upon construction and
            // adding sstable::unlink, to be called here, that will update large_data_handler
            // and delete the sstable.
    ...
    return seastar::async([ssts = std::move(ssts), &large_data_handler] {
        ...
        for (const auto& sst : ssts) {
            ...
            if (sstdir.empty()) {
                ...
                sstlog.error("Mismatching SSTable directories to be deleted atomically: {} {}", sstdir, sst->get_dir());
                return;
            }
        }
        ...
        try {
            ...
            sstlog.error("Error while writing {}: {}. Ignoring.", pending_delete_log), std::current_exception();
        }
        ...
        try {
            ...
            sstlog.error("Error removing {}: {}. Ignoring.", pending_delete_log, std::current_exception());
        }
    });
}
...
future<> replay_pending_delete_log(sstring pending_delete_log) {
    ...
    return seastar::async([pending_delete_log = std::move(pending_delete_log)] {
        ...
        try {
            ...
            sstlog.error("Error replaying {}: {}. Ignoring.", pending_delete_log, std::current_exception());
        }
    });
}
