...
static future<file> open_sstable_component_file(const io_error_handler& error_handler, sstring const& name, open_flags flags,
...
static future<file> open_sstable_component_file_non_checked(sstring const& name, open_flags flags, file_open_options options) {
    if (flags != open_flags::ro && get_config().enable_sstable_data_integrity_check()) {
...
sstring sstable::filename(sstring const& dir, sstring const& ks, sstring const& cf, version_types version, int64_t generation,
...
future<> sstable::create_links(sstring const& dir, int64_t generation) const {
