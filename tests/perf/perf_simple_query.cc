...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&app] {
        auto init = [&app] {
            ...
            auto seed = conf_seed.empty() ? std::random_device()() : conf_seed.as<random::result_type>();
            std::cout << "random-seed=" << seed << '\n';
            return smp::invoke_on_all([seed] {
                random::local_device = new random::pseudo_random_device(seed + engine().cpu_id());
                engine().at_exit([] {
                    delete random::local_device;
