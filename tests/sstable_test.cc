...
load_large_partition_sst(const sstables::sstable::version_types version, Func&& func) {
    ...
    return test_env::do_with([s = std::move(s), dir = std::move(dir), version, func = std::move(func)] (test_env& env) {
        return env.reusable_sst(std::move(s), std::move(dir), 3, version).then([func = std::move(func)] (shared_sstable sst) {
            return func(sst);
        });
    });
}
